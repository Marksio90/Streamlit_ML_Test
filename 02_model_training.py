...
def zbuduj_transformer(num: List[str], kat: List[str]) -> ColumnTransformer:
    num_tr = Pipeline(steps=[("scaler", StandardScaler(with_mean=False))])
    kat_tr = OneHotEncoder(handle_unknown="ignore", sparse_output=False)
...
def trenuj_regresje(X, y, transformer, random_state=42) -> Tuple[Pipeline, Dict[str, float]]:
    modele = {
...
def trenuj_klasyfikacje(X, y, transformer, random_state=42) -> Tuple[Pipeline, Dict[str, float]]:
    y = y.astype("category")
    modele = {
...
def trenowanie_automatyczne(df: pd.DataFrame, target: str, typ: str, random_state=42) -> Dict[str, Any]:
    X, y, num, kat = przygotuj_X_y(df, target)
    transformer = zbuduj_transformer(num, kat)

    if typ == "regresja":
        pipe, metrics = trenuj_regresje(X, y, transformer, random_state=random_state)
    else:
        pipe, metrics = trenuj_klasyfikacje(X, y, transformer, random_state=random_state)

    waznosci = policz_waznosci(pipe, X, y, random_state=random_state, n_repeats=5)
    return {
        "pipeline": pipe,
        "metrics": metrics,
        "waznosci": waznosci,
